...
#       "fine":  "2026/03/18-14:05:32:147"   <- aggiornato a ogni chiusura
#     },
#     ...
#   },
#   "targhe": [
#     {
#       "id":         "<uuid4>",           <- chiave univoca del record
#       "targa":      "AB123CD",
...
def db_load() -> dict:
    """Carica il database da disco; se non esiste lo inizializza."""
...
def db_save(db: dict) -> None:
    """Scrive il database su disco in modo atomico (write + rename)."""
    tmp = DB_PATH + ".tmp"
...
def db_open_session(db: dict, session_id: str, now: datetime) -> None:
    """Registra l'apertura di una nuova sessione."""
    db["sessioni"][session_id] = {
        "avvio": format_timestamp(now),
        "fine":  None
    }
    db_save(db)
...
def db_add_plate(db: dict, plate: str, ts: datetime, conf: float, session_id: str) -> dict:
    """
    Aggiunge un record targa al database e lo salva immediatamente.
    Restituisce il record appena inserito.
    """
    record = {
        "id":         str(uuid.uuid4()),
        "targa":      plate,
...
def db_query(db: dict, plate: str = None, session_id: str = None) -> list:
    """
    Filtra i record per targa e/o sessione.
    Senza parametri restituisce tutto l'archivio.
    """
    results = db["targhe"]
    if plate:
...
def db_print_summary(db: dict) -> None:
    """Stampa un riepilogo dell'intero archivio a fine sessione."""
    total   = len(db["targhe"])
    ...
    print(f"{'='*52}")
    for r in db["targhe"][-10:]:           # mostra le ultime 10
    ...
    for (bbox, text, conf) in result:
        ...
        if conf > 0.3:
            if plate_valid:
                ...
                # storico: quante volte questa targa è già in archivio
...
# Carica (o crea) il database all'avvio
db         = db_load()
...
# Chiudi la sessione e stampa riepilogo
